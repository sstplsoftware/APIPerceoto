...
/* ============================================================
   ⭐ ADMIN — FEEDBACK REPORT (SKILL + CAREER)
============================================================ */
router.get(
  "/results/admin/:id/feedback",
  verifyToken,
  isAdmin,
  async (req, res) => {
    try {
      ...
      if (!result) {
        return res
          .status(404)
          .json({ error: "Result not found or not owned by you" });
      }

      // 2️⃣ 🔥 STEP 4 — SKILL AGGREGATION (BRAIN)
      const skillSummary = await generateSkillAggregation(
        result.batchNumber,
        result.candidateId
      );

      // 3️⃣ Split strengths & improvement areas
      const strengths = skillSummary.filter(
        (s) => s.level === "Strong"
      );

      const improvementAreas = skillSummary.filter(
        (s) => s.level === "Needs Improvement"
      );

      // --------------------------------------------------
// 🔥 CAREER ROLE AGGREGATION (FROM mappedRoles)
// --------------------------------------------------
const questions = await Question.find({
  batchNumber: result.batchNumber,
  createdBy: req.user.id,
...
// Count role relevance
const roleMap = {};

questions.forEach((q) => {
  if (!Array.isArray(q.mappedRoles)) return;

  q.mappedRoles.forEach((role) => {
    roleMap[role] = (roleMap[role] || 0) + 1;
  });
});

// Build recommended roles (TOP 3)
const recommendedRoles = Object.entries(roleMap)
  .map(([title, count]) => ({
    title,
    reason:
      "Alignment is derived from performance in questions mapped to this role domain.",
    readiness:
      count >= 5 ? "High" : count >= 3 ? "Moderate" : "Low",
  }))
  .sort((a, b) => {
  ...
  .slice(0, 3);
      ...
      // 4️⃣ Final feedback payload (PDF-READY)
      res.json({
        candidate: {
